...
class RandomForest(object):
    ...
    def _bootstrapping(self, XX, n):
        ...
        samples = []
        labels = [] 
        index=np.random.choice(range(len(XX)),n)
        return ([samples.append(XX[i][:-1]) for i in index], [labels.append(XX[i][-1]) for i in index])
    ...
    def voting(self, X):
        ...
        for record in X:
            ...
            if len(counts) == 0:
                ...
                #during special case, use all the trees to vote since there are no out of bag trees available 
