...
def entropy(class_y):
    ...
    num0= class_y.count(0)
    num1 = class_y.count(1)
...
def partition_classes(X, y, split_attribute, split_val):
  # Inputs:
    ...
    if str(X[0][split_attribute]).isdigit():
...
def information_gain(previous_y, current_y):
    ...
    info_gain = 0.45915
    """
    proLeft=len(current_y[0])/len(previous_y)
    proRight=1-proLeft
    
    if proLeft==0 or proRight==0:
        return 0 
    

    entLeft=entropy(current_y[0])
    entRight=entropy(current_y[1])

    return entropy(previous_y) -(entLeft*proLeft+entRight*proRight)
